"""
Таблицы для алгоритма «Кузнечик» (ГОСТ 34.12-2015).
S-box (Pi), обратный S-box (Pi^-1), вектор коэффициентов C для R/L.
Источник S-box: RFC 7801 (GOST R 34.12-2015).
"""
...
# S-box Pi': Pi'(i) = значение из стандарта (RFC 7801, раздел 4.1)
SBOX: Tuple[int, ...] = (
...
# Вектор коэффициентов для линейной части R (порядок a_15..a_0 по ГОСТ)
# l(a_15,..,a_0) = 148*a_15 + 32*a_14 + 133*a_13 + ... + 1*a_0
C_VECTOR: Tuple[int, ...] = (
...
def sbox_inv_table() -> Tuple[int, ...]:
    return SBOX_INV


def c_vector() -> Tuple[int, ...]:
    return C_VECTOR


def gf_mul_table(i: int, j: int) -> int:
    """Умножение в GF(2^8) по таблице (для L и матрицы)."""
    return gf28.gf_mul_table(i, j)
